import { loadSettings as apiLoadSettings, saveSettings as apiSaveSettings, fetchJobStatus as apiFetchJobStatus, fetchSrt as apiFetchSrt } from "./api.js";
...
let editorMode = 'segments'; // 'segments' | 'text'
...
async function _loadTranscriptSrtText(srtText, displayName, { handle = null, sourceKind = 'disk' } = {}) {
  ...
  if (meta && meta.topics && Array.isArray(meta.topics) && meta.topics.length > 0) {
    console.log("Loaded embedded topics:", meta.topics.length);
...
async function autoloadFromQueryParams() {
  try {
    ...
    const banner = document.getElementById("localSaveBanner");
    const bannerSource = document.getElementById("saveLocalSource");
    const setBannerVisible = (on) => {
      if (!banner) return;
      banner.classList.toggle("hidden", !on);
    };

    // Default: hide banner until we successfully load a transcript from the server.
    setBannerVisible(false);
    if (bannerSource) bannerSource.textContent = "";
    ...
    if (srtUrl) {
      ...
      try {
        topicsView.load(srtUrl);
      } catch (e) { console.warn("Failed to load topics", e); }

      // Show banner only for server-loaded transcripts
      setBannerVisible(true);
      if (bannerSource) {
        const parts = [];
        if (tName) parts.push(tName);
        if (chosenAudioName) parts.push(chosenAudioName);
        bannerSource.textContent = parts.length ? ("Loaded from server: " + parts.join(" • ")) : "";
      }
    }
  } catch (e) {
    console.error(e);
